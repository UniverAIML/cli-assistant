...
class AnimatedEffects:
  ...
  ╚██████╗███████╗██║███║██║
   ╚═════╝╚══════╝╚═╝╚══╝╚═╝
        ...
        colors = ["bright_red", "bright_green", "bright_blue", "bright_magenta", 
    ...
    def matrix_rain(self, duration: float = 5.0) -> None:
        ...
        width = self.console.size.width
        height = self.console.size.height - 5
        ...
        with Live(auto_refresh=False) as live:
            ...
            while time.time() - start_time < duration:
                ...
                for column in rain_columns:
                    ...
                    if column['y'] > height:
                        column['y'] = -random.randint(10, 30)
                        column['x'] = random.randint(0, width - 1)
                        column['chars'] = ''.join(random.choices(chars, k=height))
                    
                    # Малюємо символи колонки
                    for i, char in enumerate(column['chars']):
                        y_pos = int(column['y'] - i)
                        if 0 <= y_pos < height and 0 <= column['x'] < width:
                            lines[y_pos] = (
                                lines[y_pos][:column['x']] + 
                                char + 
                                lines[y_pos][column['x'] + 1:]
                            )
    ...
    def spinning_loader(self, text: str = "Завантаження", duration: float = 2.0) -> None:
        ...
        with Live(auto_refresh=False) as live:
            ...
            while time.time() - start_time < duration:
                ...
                loading_text = Text(f"{spinner} {text} {spinner}", style="bold bright_cyan")
                panel = Panel(
